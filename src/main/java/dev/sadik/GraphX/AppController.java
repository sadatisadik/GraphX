...
//                }
    ...
    private void plotLine(String equation) throws Exception {
                ...
                if(isFunctionOfY) {
                    ...
                    for(double y= -range, x; y<=range; y+= STEP_SIZE) {
                        ...
                        if(!Double.isNaN(prevX) && Math.signum(x - lastX) != Math.signum(lastX - prevX) && Math.signum(lastX - prevX) != 0) {
                            setEquationGraph(currentSeries);
                    ...
                    setEquationGraph(currentSeries); // add the final points
                }
                else {
                    for(double x = -range, y; x<=range; x+=STEP_SIZE) {
    ...
    private void plotSameVariableEquation(String implicitEquation, String variable) throws Exception {
    ...
    protected void plotImplicitFunction(String implicitEquation) {
        ...
        for(double x = SEARCH_RANGE_MIN; x <= SEARCH_RANGE_MAX; x += STEP_SIZE) {
            ...
            for(Double y_root : roots) {
                ...
                for(XYChart.Series<Double, Double> existingSeries : getEquationGraphData()) {
                ...
                if(!connected) {
                    ...
                    setEquationGraph(newSeries);
                }
            }
        }
    }

    public static void setError(String err) throws Exception {
        errorMessage = err;
        AppController ob = new AppController();
        ob.showError();
    }

    public void showError() throws Exception{
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/dev/sadik/GraphX/ErrorScreen.fxml"));
        Parent root = loader.load();
        Scene errorScreen = new Scene(root);
